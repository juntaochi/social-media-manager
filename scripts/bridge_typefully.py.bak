...
def server_mode():
    """
    Run as a basic stdio MCP server without external dependencies
    """
    api_key = os.environ.get('TYPEFULLY_API_KEY')
    if not api_key:
        print("TYPEFULLY_API_KEY environment variable not set", file=sys.stderr)
        return 1
            ...
            if method == "initialize":
                ...
                send_response(request_id, {
                    "tools": [
                        {
                            "name": "upload_media",
                            "description": "Upload a media file to Typefully",
                            "inputSchema": {
                                "type": "object",
                                "properties": {
                ...
                if tool_name == "upload_media":
                    ...
                    result = uploader.upload_media(file_path)
                    send_response(request_id, {
                    ...
                    ids_list = media_ids.split(',') if media_ids else None
                    result = uploader.create_draft(content, ids_list, schedule_date)
                    send_response(request_id, {
                        "content": [{"type": "text", "text": json.dumps(result)}]
                    })
                else:
                    send_response(request_id, error={"code": -32601, "message": f"Tool not found: {tool_name}"})
